...
module ScrapeModule =
    ...
    type IgnoreBinaryAttributesResolver() =
        ...
        override this.CreateProperty(memb, membSerialization) =
            ...
            let isBinaryAttr (cad: CustomAttributeData) =
                cad.AttributeType = binaryAttributeType
    ...
    type Clip =
        { URL: string
          ...
          [<BinaryAttribute>] PreviewImage: byte array
          [<BinaryAttribute>] PreviewVideo: byte array
          [<BinaryAttribute>] PreviewGif: byte array
    ...
    let cssSelect css (node: HtmlNode) =
        node.CssSelect(css)

    /// .TryGetAttribute() but for pipes
    let tryAttribute attr (node: HtmlNode) =
        node.TryGetAttribute(attr)
    ...
    let scrapeClip url (doc: HtmlDocument) =
        let makeClip cont =
            let title =
                cont
                |> cssSelect "h3" |> Seq.tryHead
                |> Option.map HtmlNode.innerText |> Option.map trim
            let desc =
                cont
                |> cssSelect ".individualClipDescription body" |> Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.map (Seq.map string)
                |> Option.map (String.concat "\n") |> Option.map trim

            let image =
                cont
                |> cssSelect ".clipImage" |> Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 2)
                |> Option.bind (tryAttribute "src") |> Option.map absoluteUrl
                |> Option.map downloadFile
            let video =
                cont
                |> cssSelect ".clipImage" |> Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 1)
                |> Option.map HtmlNode.elements |> Option.bind Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 0)
                |> Option.bind (tryAttribute "src") |> Option.map absoluteUrl
                |> Option.map downloadFile
            let gif =
                cont
                |> cssSelect ".clipImage" |> Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 2)
                |> Option.bind (tryAttribute "data-src") |> Option.map absoluteUrl
                |> Option.map downloadFile

            let category =
                cont
                |> cssSelect ".clip_details" |> Seq.tryItem 0
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 0)
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 1)
                |> Option.map HtmlNode.innerText |> Option.map trim
            let related =
                ...
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 1)
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 1)
                |> Option.map HtmlNode.elements |> Option.defaultValue []
                |> List.map HtmlNode.elements |> List.map Seq.tryHead
                |> List.choose id
                |> List.map HtmlNode.innerText |> List.map trim
            let keywords =
                cont
                |> cssSelect ".clip_details" |> Seq.tryItem 0
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 2)
            ...
            let price =
                cont
                |> cssSelect ".clip_details" |> Seq.tryItem 1
                |> Option.map HtmlNode.elements |> Option.bind Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 0)
                |> Option.map HtmlNode.elements |> Option.bind (Seq.tryItem 0)
                |> Option.map HtmlNode.elements |> Option.bind Seq.tryHead
            ...
            | _ -> None
        Seq.tryHead (doc.CssSelect("#individualClip"))
        |> Option.bind makeClip

    /// Scrape clips from a search page.
    let rec scrapeSearch (doc: HtmlDocument) =
        let links =
            doc.Html()
            |> cssSelect ".clipWrapper" |> Seq.tryHead
            |> Option.map (cssSelect "a.clipTitleLink") |> Option.defaultValue []
            |> Seq.map (tryAttribute "href") |> Seq.choose id |> Seq.map absoluteUrl
        seq {
            for url in links do
                yield! scrape url
            ...
            let nextPage =
                doc.Html()
                |> cssSelect ".paginationNextBtn" |> Seq.tryHead
                |> Option.map HtmlNode.elements |> Option.bind Seq.tryHead
                |> Option.bind (tryAttribute "href") |> Option.map absoluteUrl
    ...
    and scrape url =
        ...
        let (|Search|_|) (doc: HtmlDocument) =
            if not (Seq.isEmpty (doc.CssSelect("#view-searchInfo"))) then Some(doc)
            else None
        let (|Clip|_|) (doc: HtmlDocument) =
            if not (Seq.isEmpty (doc.CssSelect("#individualClip"))) then Some(doc)
            else None
