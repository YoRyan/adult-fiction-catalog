...
let private retryMs = 30 * 1000

type private BinaryAttribute() =
...
let rec private downloadFile url =
    let request =
        try
...
let private cssSelect css (node: HtmlNode) = node.CssSelect(css)

/// .TryGetAttribute() but for pipes
let private tryAttribute attr (node: HtmlNode) = node.TryGetAttribute(attr)
...
let private absoluteUrl (attr: HtmlAttribute) =
...
let private scrapeClip url (doc: HtmlDocument) =
    let makeClip cont =
        let title =
            cont
            |> cssSelect "h3"
...
let rec private scrapeSearch (doc: HtmlDocument) =
    let links =
        doc.Html()
